            ...
            for o in obs[1:]:
                if np.any([o['lane_index']==obs[0]['lane_index'], o['target_lane_index']==obs[0]['lane_index'], o['lane_index']==ego_target_lane_index, o['target_lane_index']==ego_target_lane_index]):
                    ...
                    if size < 8.0:
                        ...
                        slack_penalties.append(1e6)
                        alpha_scales.append(11.0)
                    elif size < 11.0:
                        #slack_penalties.append(1e1)
                        slack_penalties.append(1e6)
                        alpha_scales.append(9.0)
                    elif size < 18.0:
                        #slack_penalties.append(2e1)
                        slack_penalties.append(1e6)
                        alpha_scales.append(7.0)
                    elif size < 25.0:
                        #slack_penalties.append(3e1)
                        slack_penalties.append(1e6)
                        alpha_scales.append(5.0)
                    else:
                        #slack_penalties.append(4e1)
                        slack_penalties.append(1e6)
                        alpha_scales.append(3.0)
            params.append((lane_change, 0.2, slack_penalties, alpha_scales,))  # (lane change, speed feedback gain, penalties for slack variables)
